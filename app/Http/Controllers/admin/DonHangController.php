    ...
    public function AjaxSuccessfulOrder(Request $request)
    {
        if($request->ajax()){
            ...
            THONGBAO::create([
                'id_tk' => $order->id_tk,
                'tieude' => 'Mã giảm giá',
                'noidung' => 'Cảm ơn bạn đã mua hàng tại Cadozvex, chúng tôi xin gửi tặng bạn mã giảm giá giảm '.$voucher->chietkhau*100 .'% cho đơn hàng từ '.number_format($voucher->dieukien, 0, '', '.').'<sup>đ</sup>. Áp dụng đến hết ngày '.$voucher->ngayketthuc.'.',
                'thoigian' => date('d/m/Y h:i'),
                'trangthaithongbao' => 0,
            ]);
            ...
            $user = TAIKHOAN::find($order->id_tk);
            if(!empty($user->device_token))
            (new PushNotificationController)->sendPush($user->device_token, "Đơn hàng", "Đơn hàng #". $request->id ." đã giao thành công. Cảm ơn bạn đã mua hàng tại Cadozvex, chúng tôi xin gửi tặng bạn mã giảm giá...");
        }
    }
